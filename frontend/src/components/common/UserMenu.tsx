...
const UserMenu = () => {
    ...
    return (
        <div ref={userMenuRef} className=" relative flex justify-evenly items-center mr-8">
            ...
            {/* drop down menu */}
            <div className={`${menuOpen ? 'block' : 'hidden'} gap-1 absolute top-16 -left-[4.4rem] flex flex-col justify-start 
                        z-50 text-white w-28 bg-black py-2 rounded-sm`}
            >
                <div className=" cursor-pointer bg-black hover:bg-grayblack py-1 px-2">Profile</div>
                <div className=" cursor-pointer bg-black hover:bg-grayblack py-1 px-2">Settings</div>
                <div className=" cursor-pointer bg-black hover:bg-grayblack py-1 px-2">Log Out</div>
            </div>
